    ...
    <div className="flex items-center gap-1.5 mb-7">
      {labels.map((label, i) => {
        ...
        return (
          <Fragment key={n}>
            <div
              className={`flex items-center gap-2 ${stage >= n ? "" : "opacity-40"}`}
            >
              <span
              ...
              <span
                className={`text-[11.5px] font-semibold ${stage >= n ? "text-ink-700" : "text-ink-400"}`}
              >
                {label}
              </span>
            </div>
            {i < 2 && (
              <div
                className={`flex-1 h-px ${stage > n ? "bg-brand" : "bg-ink-200"}`}
              />
            )}
          </Fragment>
        );
  ...
  async function sendCode() {
    ...
    try {
      ...
      if (isAxiosError(err) && err.response?.status === 429) {
        ...
        setServerError(
          "메일 서비스에 일시적인 문제가 있습니다. 잠시 후 다시 시도해주세요.",
        );
      } else {
  ...
  return (
    <div className="p-6">
      ...
      {serverError && (
        <p
          role="alert"
          className="text-[12.5px] text-danger bg-danger/5 rounded-card px-3 py-2 mb-4"
        >
          {serverError}
        </p>
      )}
        ...
        <div>
          ...
          <div className="mt-7 flex flex-col gap-3">
            <div>
              <label
                htmlFor="reset-email"
                className="block text-[12.5px] font-semibold text-ink-700 mb-1.5"
              >
                이메일
              </label>
          ...
          <div className="mt-6 flex items-start gap-2.5 text-[12px] text-ink-500 bg-ink-100/60 border border-ink-200 rounded-card p-3.5">
            ...
            <span>
              소셜 계정(카카오·Google·Apple)으로 가입하셨다면, 해당 서비스에서
              비밀번호를 관리합니다.
            </span>
          </div>
        </div>
      ...
      {stage === 2 && (
        <div>
          ...
          <p className="text-[13.5px] text-ink-500 mt-1.5 leading-relaxed">
            <b className="text-ink font-semibold">{email}</b>으로
            <br />
            보낸 6자리 코드를 입력해 주세요.
          </p>
          <div className="mt-7">
            ...
            {codeErr && (
              <p
                role="alert"
                className="text-[12px] text-danger mt-2.5 flex items-center gap-1.5"
              >
                <AlertIcon />
                {codeErr}
              </p>
            ...
            <div className="flex items-center justify-between mt-4">
              <span className="text-[12.5px] text-ink-400">
                남은 시간{" "}
                <b
                  className={`tabular-nums font-bold ${secs > 0 ? "text-brand" : "text-danger"}`}
                >
                  {mmss}
                </b>
              </span>
      ...
      {stage === 4 && (
        <div className="text-center py-4">
          ...
          <h1 className="text-[21px] font-extrabold text-ink tracking-tight">
            비밀번호가 변경됐어요
          </h1>
