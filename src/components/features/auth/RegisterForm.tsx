...
function RegisterFormInner() {
  ...
  const strengthColor =
    strength === 3 ? "bg-ok" : strength === 2 ? "bg-warn" : "bg-danger";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setServerError("");
    try {
      const { pendingToken } = await signup({
        email,
        password,
        consents,
        ageConfirmed,
  ...
  return (
    ...
    >
      ...
      <div className="mb-1">
        <h1 className="text-[20px] font-extrabold text-ink tracking-tight">
          이메일로 가입하기
        </h1>
      ...
      <div className="space-y-1.5">
        ...
        <div className="relative">
          ...
          <input
            ...
            aria-describedby={
              passwordError ? "reg-password-error" : "reg-password-hint"
            }
            placeholder="비밀번호"
            className="w-full border border-ink-200 rounded-input pl-9 pr-9 py-2.5 text-[13.5px] text-ink placeholder:text-ink-400 focus:outline-none focus:border-brand aria-[invalid=true]:border-danger"
          />
        ...
        {passwordError ? (
          ...
          <p id="reg-password-hint" className="text-[11.5px] text-ink-400">
            ✓ 영문·숫자 포함 8자 이상
          </p>
        )}
      </div>
      ...
      <div className="space-y-1">
        ...
        <div className="relative">
          ...
          <button
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 text-ink-400 hover:text-ink"
            aria-label={
              showConfirm ? "비밀번호 확인 숨기기" : "비밀번호 확인 보기"
            }
          >
