    ...
    >
      ...
      <div className="space-y-1.5">
        ...
        <p className="text-[13.5px] text-ink-600 leading-relaxed">
          <span className="font-bold text-ink">{email}</span>으로
          <br />
          보낸 6자리 코드를 입력해 주세요.
        </p>
      </div>
      ...
      {successMessage && (
        <p
          role="status"
          className="text-sm text-ok bg-ok/10 rounded-input px-3 py-2"
        >
          {successMessage}
        </p>
      )}
      ...
      <div className="flex items-center justify-between text-[12.5px]">
        <span className="text-ink-400">
          남은 시간{" "}
          <span
            className={`font-bold tabular-nums ${secs <= 30 ? "text-danger" : "text-brand"}`}
          >
            {fmt(secs)}
          </span>
        </span>
