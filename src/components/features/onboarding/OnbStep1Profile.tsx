...
export default function OnbStep1Profile({
  nick,
  age,
  job,
  dispatch,
}: OnbStep1ProfileProps) {
  ...
  return (
    <div className="flex flex-col gap-7">
      <div>
        <label
          htmlFor="onb-nick"
          className="block text-[13px] font-bold text-ink-700 mb-2.5"
        >
        ...
        <div className="relative">
          <input
            ...
            value={nick}
            onChange={(e) =>
              dispatch({ type: "SET_NICK", value: e.target.value.slice(0, 12) })
            }
        ...
        {nickError && (
          <p
            id="onb-nick-error"
            role="alert"
            className="mt-1.5 text-[12px] text-danger font-medium"
          >
            닉네임을 입력해주세요
          </p>
        )}
