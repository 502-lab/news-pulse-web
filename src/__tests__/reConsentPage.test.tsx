...
const mockMyConsents = [{ termsVersionId: "svc-1", agreed: true }];
...
describe("ReConsentPage — S-6 약관 재동의", () => {
  beforeEach(() => {
    server.use(
      ...
      http.get(`${BASE}/api/v1/me/consents`, () =>
        HttpResponse.json(mockMyConsents),
      ),
      http.post(`${BASE}/api/v1/me/consents`, () => HttpResponse.json([])),
      http.get(`${BASE}/api/v1/me`, () =>
        HttpResponse.json({ ...mockUserReConsent, requiresReConsent: false }),
      ),
    );
  });
  ...
  it('"거부하고 로그아웃" 클릭 → /login 이동', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("button", { name: "거부하고 로그아웃" }),
      ).toBeInTheDocument();
    });
  ...
  it("약관 동의 후 제출 → requiresReConsent=false → /home 이동", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("checkbox", { name: "전체 동의" }),
      ).toBeInTheDocument();
    });
