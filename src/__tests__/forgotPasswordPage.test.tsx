...
describe('ForgotPasswordPage — S-5 비밀번호 재설정 4단계', () => {
  ...
  it('Stage1: 이메일 입력 후 발송 → Stage2 전환', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('heading', { name: '인증 코드 입력' })).toBeInTheDocument();
    });
  ...
  it('전체 4단계 흐름: Stage1 → Stage2 → Stage3 → Stage4(성공)', async () => {
    ...
    // Stage 2
    await waitFor(() => expect(screen.getByRole('heading', { name: '인증 코드 입력' })).toBeInTheDocument());
    ...
    // Stage 4
    await waitFor(() =>
      expect(screen.getByRole('heading', { name: '비밀번호가 변경됐어요' })).toBeInTheDocument(),
    );
  ...
  it('Stage2: 잘못된 코드 → 400 → 코드 오류 표시', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('인증 코드가 올바르지 않습니다. 다시 확인해 주세요.')).toBeInTheDocument();
    });
  });
  ...
  it('Stage3: 비밀번호 불일치 → 오류 표시 + 제출 차단', async () => {
    ...
    await waitFor(() => expect(screen.getByLabelText('새 비밀번호')).toBeInTheDocument());
    await user.type(screen.getByLabelText('새 비밀번호'), 'NewPass1');
    await user.type(screen.getByLabelText('새 비밀번호 확인'), 'DifferentPass2');
  ...
  it('Stage2: 이메일 주소 바꾸기 → Stage1으로 복귀', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('heading', { name: '인증 코드 입력' })).toBeInTheDocument());
    await user.click(screen.getByRole('button', { name: '이메일 주소 바꾸기' }));
