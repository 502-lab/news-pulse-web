...
describe("BreakingBanner", () => {
  it("renders title and source", () => {
    ...
    expect(
      screen.getByText(new RegExp(mockArticle.source)),
    ).toBeInTheDocument();
  });
...
describe("CategoryFilter", () => {
  ...
  it("active button has aria-pressed=true", () => {
    ...
    expect(screen.getByRole("button", { name: "기술" })).toHaveAttribute(
      "aria-pressed",
      "true",
    );
...
describe("TrendingKeywordsWidget", () => {
  it("renders all keywords", () => {
    wrap(
      <TrendingKeywordsWidget keywords={TRENDING_KEYWORDS} onPick={vi.fn()} />,
    );
  ...
  it("calls onPick with keyword when button clicked", () => {
    const onPick = vi.fn();
    wrap(
      <TrendingKeywordsWidget keywords={TRENDING_KEYWORDS} onPick={onPick} />,
    );
