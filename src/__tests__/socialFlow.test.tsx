...
describe("소셜 콜백 — 카카오 신규 유저 (202 → consent → /home)", () => {
  beforeEach(() => {
    ...
    server.use(
      http.post(
        `${BASE}/api/v1/auth/social/kakao/callback`,
        () =>
          new HttpResponse(
            JSON.stringify({
              isNew: true,
      ...
      http.post(`${BASE}/api/v1/auth/social/complete`, () =>
        HttpResponse.json(
          { account: mockUser, tokens: mockTokens },
          { status: 201 },
        ),
      ),
    );
    ...
    const routes = [
      {
        ...
        children: [
          ...
          {
            path: "/social-consent",
            element: <div data-testid="consent-page">Consent</div>,
          },
        ],
      },
    ];
  ...
  it("SocialConsentPage: 약관 동의 후 completeSocialSignup → /home", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("heading", { name: /가입을 완료하려면/i }),
      ).toBeInTheDocument();
    });
