...
const SHADOW_TOKENS = [
  {
    name: "shadow-card",
    val: "0 1px 2px rgba(15,23,42,.04), 0 1px 3px rgba(15,23,42,.06)",
    use: "카드 기본",
  },
  {
    name: "shadow-cardhover",
    val: "0 2px 4px rgba(15,23,42,.06), 0 4px 12px rgba(15,23,42,.08)",
    use: "카드 hover",
  },
  {
    name: "shadow-pop",
    val: "0 8px 28px rgba(15,23,42,.16)",
    use: "모달·드롭다운·토스트",
  },
];
...
const MOCK_ARTICLE = {
  ...
  cat: "기술",
  title:
    "삼성전자, 차세대 2nm GAA 공정 양산 공식화… 파운드리 점유율 반등 노린다",
  source: "연합뉴스",
  time: "12분 전",
  bias: -6,
  reads: 24100,
...
function Section({
  ...
  return (
    <section id={id} className="mb-14 scroll-mt-20">
      ...
      {desc && (
        <p className="text-[13.5px] text-ink-500 mb-6 ml-10 max-w-2xl leading-relaxed">
          {desc}
        </p>
      )}
...
function Demo({
  ...
  return (
    <div className="mb-6">
      {label && (
        <div className="text-[11px] font-bold uppercase tracking-wider text-ink-400 mb-2">
          {label}
        </div>
      )}
      <div
        className={`rounded-card border border-ink-200 p-5 flex flex-wrap items-center gap-4 ${dark ? "bg-navy" : "bg-canvas"}`}
      >
        {children}
      </div>
    </div>
...
function SpecLine({
  ...
  return (
    <div className="mb-3 flex flex-wrap gap-x-6 gap-y-1.5 text-[12px]">
      <span className="font-mono font-bold text-ink bg-ink-100 rounded px-2 py-0.5">
        {name}
      </span>
      {variants && (
        <span className="text-ink-500">
          <b className="text-ink-700">variants:</b> {variants}
        </span>
      )}
      {states && (
        <span className="text-ink-500">
          <b className="text-ink-700">states:</b> {states}
        </span>
      )}
      {props && (
        <span className="text-ink-500">
          <b className="text-ink-700">props:</b>{" "}
...
function SegDemo() {
  ...
  return (
    <Segmented value={v} onChange={setV} options={["오늘", "7일", "30일"]} />
  );
}
      ...
      <div className="max-w-[1080px] mx-auto px-8 py-10 flex gap-10">
        ...
        <main className="flex-1 min-w-0">
          ...
          <p className="text-[13.5px] text-ink-600 leading-relaxed mb-10 bg-brand-50/50 border border-brand/15 rounded-card p-4">
            이 페이지는 <b>실제 프로젝트에서 사용되는</b> 공통 컴포넌트와 디자인
            토큰 카탈로그입니다. 각 컴포넌트의{" "}
            <b>variant · size · 상태 · props</b>를 함께 표기했어요.
          </p>
          ...
          >
            {COLORS.map(({ group, rows }) => (
              <div key={group} className="mb-6">
                <div className="text-[12.5px] font-bold text-ink-600 mb-2.5">
                  {group}
                </div>
                <div className="grid grid-cols-2 gap-2.5">
                  {rows.map(({ name, hex, use }) => (
                    <div
                      key={name}
                      className="flex items-center gap-3 border border-ink-200 rounded-btn p-2"
                    >
                      <span
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="font-mono text-[11px] text-ink-400 uppercase">
                          {hex} ·{" "}
                          <span className="font-sans normal-case">{use}</span>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <tbody>
                  {TYPE_SCALE.map(([tok, size, weight, lh, use]) => (
                    <tr key={tok} className="border-t border-ink-100">
                      ...
                      <td className="px-3 py-3 font-mono text-[12px] text-ink-500 tnum align-top">
                        {size}px
                      </td>
                      <td className="px-3 py-3 font-mono text-[12px] text-ink-500 tnum align-top">
                        {weight}
                      ...
                      <td
                        className="px-3 py-3 text-ink align-top"
                        style={{
          ...
          >
            <Demo label="Border radius">
              {RADIUS_TOKENS.map(({ name, px, use }) => (
                <div key={name} className="text-center">
                  ...
                  />
                  <div className="text-[11px] font-semibold text-ink mt-2">
                    {name}
                  </div>
            ...
            <Demo label="Shadow / elevation">
              {SHADOW_TOKENS.map(({ name, val, use }) => (
                <div key={name} className="text-center">
                  <div
                    className="w-16 h-16 bg-white rounded-card"
                    style={{ boxShadow: val }}
                  />
                  <div className="text-[11px] font-semibold text-ink mt-3">
                    {name}
                  </div>
          ...
          >
            <SpecLine
              name="<Icon>"
              props="name, size=18, stroke=2, className"
              note="currentColor 상속"
            />
            <Demo>
              {ALL_ICONS.map((n) => (
                <div
                ...
                >
                  ...
                  <span className="font-mono text-[9.5px] text-ink-400 text-center break-all">
                    {n}
                  </span>
                </div>
              ))}
          ...
          >
            <SpecLine
              name="<CatBadge>"
              variants="기술·경제·정치·스포츠·문화"
              props="cat, sm"
            />
            <SpecLine
              name="<BiasChip>"
              variants="진보(<-10) · 중립 · 보수(>10)"
              props="score(-100..100)"
            />
            <SpecLine
              name="<StatusDot>"
              variants="running · idle · warning · error"
              props="status"
            />
            <Demo>
          ...
          >
            <SpecLine
              name="<Segmented>"
              props="options, value, onChange, size='md'|'sm'"
              note="배경 pill 슬라이드"
            />
            <Demo>
              <SegDemo />
              <div className="inline-flex p-0.5 bg-ink-100 rounded-btn">
                <span className="rounded-[5px] px-3 py-1 text-[12.5px] font-semibold bg-white text-ink shadow-sm">
                  USER
                </span>
                <span className="rounded-[5px] px-3 py-1 text-[12.5px] font-semibold text-ink-500">
                  ADMIN
                </span>
              </div>
            </Demo>
            <Demo label="필터 칩 탭">
              <span className="px-3.5 py-1.5 rounded-btn text-[13px] font-semibold bg-brand text-white">
                전체
              </span>
              {["기술", "경제", "정치"].map((c) => (
                <span
                  key={c}
                  className="px-3.5 py-1.5 rounded-btn text-[13px] font-semibold bg-white border border-ink-200 text-ink-500"
                >
                  {c}
                </span>
              ))}
            ...
            <Demo label="언더라인 탭">
              <div className="flex gap-0">
                ...
                <span className="px-3.5 py-2 text-[14px] font-bold text-ink-400">
                  트렌드
                </span>
                <span className="px-3.5 py-2 text-[14px] font-bold text-ink-400">
                  편향 분석
          ...
          >
            <SpecLine
              name="<Card> / <CardHead>"
              props="pad=true, hover=false / title, sub, right"
            />
            <SpecLine
              name="<StatCard>"
            ...
            <div className="max-w-xs mb-4">
              <Card hover>
                <CardHead
                  ...
                  sub="Subtitle"
                  right={
                    <span className="text-[11px] text-ink-400">우측 영역</span>
                  }
                />
            ...
            <SpecLine
              name="<NewsCard>"
              props="a(article), onOpen, dense"
              note="썸네일+뱃지+제목+메타+북마크"
            />
          ...
          >
            <SpecLine
              name="showToast(msg, {icon, tone})"
              note="ToastHost를 앱 루트에 1개 마운트"
            />
          ...
          <footer className="border-t border-ink-200 pt-6 mt-12 text-[12px] text-ink-400">
            Newsift Component Library · 프로젝트 공통 컴포넌트 카탈로그 · 웹
            전용
          </footer>
        </main>
