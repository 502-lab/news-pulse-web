...
import { getActiveTerms, getConsents, submitConsents, TERMS_QUERY_KEYS } from '@/lib/api/terms';
...
export default function ReConsentPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: TERMS_QUERY_KEYS.myConsents });
      if (user && accessToken) {
        // 로컬 갱신 먼저
  ...
  async function handleDecline() {
    ...
    if (rt) {
      try { await logoutApi(rt); } catch { /* 서버 오류 무관 */ }
    }
    logout();
  ...
  const pendingTerms = (terms ?? []).filter((t) => t.isRequired && !agreedIds.has(t.id));

  const requiredIds = pendingTerms.filter((t) => t.isRequired).map((t) => t.id);
  const allRequiredAgreed =
    requiredIds.length === 0 ||
    (requiredIds.every((id) => consents.find((c) => c.termsVersionId === id)?.agreed) &&
      ageConfirmed);
