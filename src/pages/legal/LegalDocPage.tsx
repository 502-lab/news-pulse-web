    ...
    <div className="min-h-screen bg-canvas">
      ...
      <div className="max-w-[820px] mx-auto px-6 py-12 fadeup">
        <div className="flex items-center gap-2 text-[12px] font-bold text-brand mb-3">
          <span className="bg-brand-50 px-2 py-0.5 rounded tnum">
            {doc.code}
          </span>
          <span className="uppercase tracking-wider">{doc.en}</span>
        </div>

        <h1 className="display text-[32px] font-extrabold text-ink tracking-tight">
          {doc.title}
        </h1>
        ...
        <div className="flex items-center gap-2 mt-3 text-[12.5px] text-ink-400">
          <ClockIcon />
          <span>
            최종 개정일 <span className="tnum">{doc.updated}</span> · 시행
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-1.5 my-7 p-5 bg-white border border-ink-200 rounded-card shadow-card">
          ...
          {doc.sections.map((s, i) => (
            <div
              key={i}
              className="flex items-center gap-2 text-[13px] text-ink-600 py-0.5"
            >
            ...
            <section key={i}>
              ...
              <div className="mt-3 flex flex-col gap-2.5 pl-[34px]">
                {s.body.map((p, j) => (
                  <p
                    key={j}
                    className="text-[13.5px] text-ink-600 leading-[1.75] flex gap-2.5"
                  >
                    <span
                      className="text-ink-300 shrink-0 mt-px"
                      aria-hidden="true"
                    >
                      ·
                    </span>
